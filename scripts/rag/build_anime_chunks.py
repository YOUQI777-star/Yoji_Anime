...
IN_FILE = DATA_DIR / "anime_docs.jsonl"
OUT_FILE = DATA_DIR / "anime_chunks.jsonl"
...
with open(IN_FILE, "r", encoding="utf-8") as fin, open(OUT_FILE, "w", encoding="utf-8") as fout:
    for line in fin:
        ...
        if not overview_text:
            overview_bits = [f"作品名：{title}。"]
            if title_ja and title_ja != title:
                overview_bits.append(f"日文名：{title_ja}。")
            if title_cn and title_cn != title:
                overview_bits.append(f"中文名：{title_cn}。")
            if country:
                overview_bits.append(f"国家/地区：{country}。")
            if platform:
                overview_bits.append(f"播出形式：{platform}。")
            if date:
                overview_bits.append(f"首播时间：{date}。")
            if episodes not in [None, ""]:
                overview_bits.append(f"集数：{episodes}。")
            if studio:
                overview_bits.append(f"制作公司：{studio}。")
            if director:
                overview_bits.append(f"导演：{director}。")
            if score not in [None, ""]:
                overview_bits.append(f"评分：{score}。")
            if rank not in [None, ""]:
                overview_bits.append(f"排名：{rank}。")
...
print(f"anime_chunks written: {chunks_written}")
